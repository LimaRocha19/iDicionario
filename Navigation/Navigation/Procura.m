...
#warning Incomplete method implementation.
    ...
    return dicionario.alphabet.count + 1;
}


- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    Palavra *cell = [self.tabela dequeueReusableCellWithIdentifier:@"celulaPadrao" forIndexPath:indexPath];
    
    if (indexPath.row < 26) {
    
        cell.Letra.text = [dicionario.alphabet objectAtIndex:indexPath.row];
        cell.Word.text = [dicionario.examples objectAtIndex:indexPath.row];
    }
    else {
        cell.Letra.text = @" ";
        cell.Word.text = @" ";
